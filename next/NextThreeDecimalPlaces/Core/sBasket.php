...
namespace NextThreeDecimalPlaces\Core;
        ...
        if (!empty($result['content'])) {
            foreach ($result['content'] as $key => $value) {
                if (!empty($value['amountWithTax'])) {
                    $t = round(str_replace(',', '.', $value['amountWithTax']), 3);
                } else {
                    $t = str_replace(',', '.', $value['price']);
                    $t = floatval(round($t * $value['quantity'], 3));
                }
                if (!$this->sSYSTEM->sUSERGROUPDATA['tax'] && $this->sSYSTEM->sUSERGROUPDATA['id']) {
                    $p = floatval($this->moduleManager->Articles()->sRound(
                        $this->moduleManager->Articles()->sRound(
                            round($value['netprice'], 3) * $value['quantity'])
                        )
                    );
                } else {
                    $p = floatval($this->moduleManager->Articles()->sRound(
        ...
        foreach (array_keys($getArticles) as $key) {
            ...
            if (
                ...
                if (empty($getArticles[$key]['modus'])) {
                    $priceWithTax = round($netprice, 3) / 100 * (100 + $tax);
                    ...
                    $getArticles[$key]['amountWithTax'] = round(1 * (round($price, 2) / 100 * (100 + $tax)), 3);
                    // Basket discount
                    ...
                    $getArticles[$key]['amountWithTax'] = round(1 * ($price / 100 * (100 + $tax)), 3);
                    if ($this->sSYSTEM->sUSERGROUPDATA['basketdiscount'] && $this->sCheckForDiscount()) {
                        $discount += ($getArticles[$key]['amountWithTax'] / 100 * $this->sSYSTEM->sUSERGROUPDATA['basketdiscount']);
                    }
                }
            ...
            $getArticles[$key]['amount'] = $quantity * round($price, 3);
            ...
            if ($getArticles[$key]['modus'] == 2) {
                // Gutscheine
                if (!$this->sSYSTEM->sUSERGROUPDATA['tax'] && $this->sSYSTEM->sUSERGROUPDATA['id']) {
                    $getArticles[$key]['amountnet'] = $quantity * round($price, 3);
                } else {
                    $getArticles[$key]['amountnet'] = $quantity * round($netprice, 3);
                }
            } else {
                if (!$this->sSYSTEM->sUSERGROUPDATA['tax'] && $this->sSYSTEM->sUSERGROUPDATA['id']) {
                    $getArticles[$key]['amountnet'] = $quantity * round($netprice, 3);
            ...
            $totalAmount += round($getArticles[$key]['amount'], 3);
            // Needed if shop is in net-mode
            $totalAmountWithTax += round($getArticles[$key]['amountWithTax'], 3);
            // Ignore vouchers and premiums by counting articles
            if (!$getArticles[$key]['modus']) {
                ++$totalCount;
            }

            $totalAmountNet += round($getArticles[$key]['amountnet'], 3);
