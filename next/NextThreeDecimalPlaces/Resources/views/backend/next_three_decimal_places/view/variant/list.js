...
//{$smarty.block.parent}
Ext.define('Shopware.apps.NextThreeDecimalPlaces.view.variant.List', {
    ...
    */
    ...
    createCellEditor: function() {
        ...
        me.cellEditor.on('edit', function(editor, e) {
            if (e.value && e.field === 'price') {
                ...
                newPrice = Ext.Number.toFixed(newPrice, 3);

                var oldPrice = Ext.Number.from(e.originalValue);
                oldPrice = Ext.Number.toFixed(oldPrice, 3);
                ...
                var newPseudoPrice = Ext.Number.toFixed(0, 3);
                var oldPseudoPrice = null;

                if(e.value !== null) {
                    newPseudoPrice = Ext.Number.from(e.value);
                    newPseudoPrice = Ext.Number.toFixed(newPseudoPrice, 3);
                    ...
                    oldPseudoPrice = Ext.Number.toFixed(oldPseudoPrice, 3);
                }
    ...
    priceColumnRenderer: function(value, metaData, record) {
        ...
        if (prices.getCount() === 0) {
            ...
            if (prices.getCount() === 1) {
                 return Ext.util.Format.number(firstPrice.get('price'),'0,00.000');
            } else {
                return me.snippets.columns.price.from + ' ' + Ext.util.Format.number(firstPrice.get('price'),'0,00.000');
            }
        }
    },
    ...
    pseudoPriceColumnRenderer: function(value, metaData, record) {
        ...
        var firstPrice = prices.first();
        return Ext.util.Format.number(firstPrice.get('pseudoPrice'),'0,00.000');
    },
